...
ParticleShaderProgram::ParticleShaderProgram(SPHSolver *solver,
  ...
  // Get uniform IDs
  uBillboardTextureSamplerID =
    glGetUniformLocation(programID, "u_BillboardTextureSampler");
  ...
  std::cout << "DEBUG:SHADER: Program " << programID <<
    " is type particles" << std::endl;
  #endif
  #if MFluidSolver_LOG_LEVEL <= MFluidSolver_LOG_WARN
  if (aBillboardVertexArrID == -1) {
    std::cout << "WARN: avs_Billboard is not bound for program " <<
      programID << std::endl;
  }
  if (uBillboardTextureSamplerID == -1) {
    std::cout << "WARN: u_BillboardTextureSampler is not bound for program " <<
      programID << std::endl;
  }
  if (uCameraRightVecID == -1) {
    std::cout << "WARN: u_CameraRight is not bound for program " <<
      programID << std::endl;
  }
  if (uCameraUpVecID == -1) {
    std::cout << "WARN: u_CameraUp is not bound for program " <<
      programID << std::endl;
  }
  if (uParticleSizeFloatID == -1) {
    std::cout << "WARN: u_ParticleSize is not bound for program " <<
      programID << std::endl;
  }
  #endif
}
...
void ParticleShaderProgram::draw() {
  ...
  for (unsigned int i = 0; i < particles.size(); ++i) {
    particleColorArray[i] = glm::vec3(particles[i].color);
    particlePositionArray[i] = particles[i].position();
  }
  ...
  if (particleColorArrBufferID != -1) {
    glBindBuffer(GL_ARRAY_BUFFER, particleColorArrBufferID);
    glBufferData(GL_ARRAY_BUFFER,  // Buffer orphaning for performance
      solver->maxParticles() * sizeof(glm::vec3), NULL, GL_STREAM_DRAW);
    glBufferSubData(GL_ARRAY_BUFFER, 0,
        solver->numParticles() * sizeof(glm::vec3), particleColorArray);
  }
  if (particlePositionArrBufferID != -1) {
    glBindBuffer(GL_ARRAY_BUFFER, particlePositionArrBufferID);
    glBufferData(GL_ARRAY_BUFFER,  // Buffer orphaning for performance
      solver->maxParticles() * sizeof(glm::vec3), NULL, GL_STREAM_DRAW);
    glBufferSubData(GL_ARRAY_BUFFER, 0,
      solver->numParticles() * sizeof(glm::vec3), particlePositionArray);
  }
  ...
  // Disable attributes
  if (aBillboardVertexArrID != -1)
    glDisableVertexAttribArray(aBillboardVertexArrID);
