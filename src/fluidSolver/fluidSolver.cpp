...
FluidSolver::FluidSolver()
    : _maxParticles(MFluidSolver_DEFAULT_MAX_PARTICLES),
      _gravity(MFluidSolver_DEFAULT_GRAVITY),
      ...
      visualizationVelocityColor(
        static_cast<float>(
          MFluidSolver_DEFAULT_VISUALIZATION_VELOCITYCOLOR_R / 255.f),
        static_cast<float>(
          MFluidSolver_DEFAULT_VISUALIZATION_VELOCITYCOLOR_G / 255.f),
        static_cast<float>(
...
void FluidSolver::printPerformanceStats() {
  if (numUpdates > 0) {
    ...
    std::cout << "PERF: Overall simulation ran for " <<
      MUtils::toHMS(computeTime) << " (" << computeTime << " seconds) over "
      << numUpdates << " frames" << std::endl;
    std::cout << "PERF: Average is " <<
      (computeTime / numUpdates) << " seconds per frame" << std::endl;
    std::cout << "PERF: Simulated " <<
      (numUpdates * _fixedTimestep) << " seconds of fluid simulation" <<
      std::endl;
  }
}
