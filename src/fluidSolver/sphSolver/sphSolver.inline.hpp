...
#ifndef MFLUIDSOLVER_SPHSOLVER_INLINE_HPP_
#define MFLUIDSOLVER_SPHSOLVER_INLINE_HPP_

#if MFluidSolver_USE_TBB
  #define iter_all_sphparticles_start \
  tbb::parallel_for((size_t)0, _particles.size(), \
    [&](size_t i) { \
...
inline void SPHSolver::calculateNonPressureForce(SPHParticle *p) {
  ...
  if (muViscosity > 0) {
    for (SPHParticle *n : *(p->neighbors())) {
      viscosityForce += glm::vec3(
        n->mass() * (n->velocity() - p->velocity()) *
        static_cast<float>(kernelFunctions.computeViscousLaplacian(
...
inline void SPHSolver::visualizeParticle(SPHParticle *p) {
  switch (visualizationType) {
    case FluidVisualizationType::Density: {
      float densityNormalized = (p->density() - dRestDensity) /
                                visualizationMaxDensityDifference;
      if (densityNormalized > 1.f) densityNormalized = 1.f;
      if (densityNormalized < -1.f) densityNormalized = -1.f;
      if (densityNormalized >= 0) {
        p->color = (1.f - densityNormalized) * glm::vec3(1.f) +
                  densityNormalized * glm::vec3(0, 1, 0);
      } else {
        densityNormalized *= -1;
        p->color = (1.f - densityNormalized) * glm::vec3(1.f) +
                  densityNormalized * glm::vec3(1, 0, 0);
      }
      break;
    }
    case FluidVisualizationType::Index: {
      float indexNormalized = static_cast<float>(p->index) /
                              static_cast<float>(largestIndex);
      p->color = (1.f - indexNormalized) * glm::vec3(1.f) +
                indexNormalized * glm::vec3(1, 0, 0);
      break;
    }
    case FluidVisualizationType::Pressure: {
      ...
      if (pressureNormalized >= 0) {
        p->color = (1.f - pressureNormalized) * glm::vec3(1.f) +
                  pressureNormalized * glm::vec3(0, 1, 0);
      } else {
        pressureNormalized *= -1;
        p->color = (1.f - pressureNormalized) * glm::vec3(1.f) +
                  pressureNormalized * glm::vec3(1, 0, 0);
      }
      break;
    }
    case FluidVisualizationType::Velocity: {
      float velocityNormalized = glm::length(p->velocity()) /
                                 visualizationMaxVelocity;
      if (velocityNormalized > 1.f) velocityNormalized = 1.f;
      p->color = (1.f - velocityNormalized) * visualizationVelocityColor +
                velocityNormalized * glm::vec3(1.f);
      break;
    }
...
#endif  // MFLUIDSOLVER_SPHSOLVER_INLINE_HPP_
